...
int main()
{
	...
	*/
	...
	// TODO: check this!
	av_opt_set(pOutCodecContext->priv_data, "preset", "slow", 0); // slow or fast ???
	/*
				...
				if (response >= 0) // = 0 (success) ???
				{
					...
					*/
					...
					while (response >= 0 && keepRunning)
					{
						...
						if (response == AVERROR(EAGAIN) || response == AVERROR_EOF)
						{
							cout << "-> BREAKED! (avcodec_receive_packet)" << endl;
							break;
						}
						else if (response < 0)
						{
						...
						/*
						cout << "Output packet PTS: " << pOutPacket->pts << endl;
						cout << "Output packet DTS: " << pOutPacket->dts << endl;
						...
						*/						
						...
						// TODO: check this!
						response = av_interleaved_write_frame(pOutFormatContext, pOutPacket); // why do I have pInPacket in the example ???
						if (response < 0)
						{
